...
class StreamingIOTest(object):
    ...
    def pre_config(self, output_path):
        ...
        self._input_gen=subprocess.Popen([join(dirname(__file__),"./gen"),
                                            input_file_name])
        self._output_gen=subprocess.Popen([join(dirname(__file__),"./val"),
                                             output_file_name])
    ...
    def post_check(self, output_path):
        ...
        assert self._output_gen.wait()==0, "Output val failed"

        return True

streamio = StreamingIOTest()
lib.test_bench("example_tb").set_pre_config(streamio.pre_config)
lib.test_bench("example_tb").set_post_check(streamio.post_check)
